...
func (s *Store) GetAllProducts(ctx context.Context) ([]models.Product, error) {
	rows, err := s.querier(ctx).Query(ctx, "SELECT p.id, p.uuid, p.name, p.description, p.price_from, p.price_to, p.type, p.file_id, f.content, p.user_id, p.created_at, p.updated_at FROM products p JOIN files f ON f.id = p.file_id ORDER BY p.created_at DESC")
	if err != nil {
...
func (s *Store) UpdateProduct(ctx context.Context, product *models.Product) error {
	_, err := s.querier(ctx).Exec(ctx, "UPDATE products SET name = $1, description = $2, price_from = $3, price_to = $4, type = $5, file_id = $6, user_id = $7, updated_at = $8 WHERE id = $9", product.Name, product.Description, product.PriceFrom, product.PriceTo, product.Type, product.FileID, product.UserID, product.UpdatedAt, product.ID)
	return wrapDBError(err)
}
