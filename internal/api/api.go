...
func (s *Service) getRoutes() *http.ServeMux {
	...
	mux.HandleFunc("GET /api/categories/{slug}", s.auth(s.getCategory))
	mux.HandleFunc("POST /api/categories", s.auth(s.createCategory))
	mux.HandleFunc("PATCH /api/categories/{slug}", s.auth(s.updateCategory))
	mux.HandleFunc("DELETE /api/categories/{slug}", s.auth(s.deleteCategory))
